---
title: Installation
description: Install doraval on your machine.
sidebar:
  order: 2
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Choose the method that matches your setup.

## macOS (Homebrew)

Fastest path on macOS, no Node or Bun runtime required:

```bash
brew tap saif-shines/tap
brew trust saif-shines/tap
brew install doraval
```

On some systems you must run `brew trust` for third-party taps (run it if `brew install` complains about an untrusted tap).

You can also use the formula form:

```bash
brew trust --formula saif-shines/tap/doraval
```

Verify:

```bash
doraval --help
```

## npm / npx

Works anywhere Node.js is installed. No separate Bun install needed.

```bash
npx @hacksmith/doraval validate .
# if Bun is not installed: npm i -g bun
```

Or install globally once:

```bash
npm install -g @hacksmith/doraval
doraval validate .
```

The wrapper detects whether Bun is available and uses it automatically. If Bun is not installed, it will prompt you to install it.

## Bun

Don't have Bun? Install it first:

```bash
curl -fsSL https://bun.sh/install | bash   # macOS / Linux
```

For Windows, see [bun.sh/docs/installation](https://bun.sh/docs/installation). Requires Bun **v1.2+**.

Then run doraval:

```bash
bunx @hacksmith/doraval validate .
```

Or install globally:

```bash
bun add -g @hacksmith/doraval
doraval validate .
```

## Verify

```bash
doraval --help
```

Expected output shows the top-level help with primary commands including `scan`, `review`, `fix`, `new`, `memory`, `reconcile`, `config`, `sessions`, plus Advanced provider groups.

After install, run `doraval update --check` to confirm you are on the latest version (see the [update command](/commands/update/)).

## JSR (library use only)

doraval is also published on [JSR](https://jsr.io/@hacksmith/doraval) for importing as a module in Bun projects:

```bash
bunx jsr add @hacksmith/doraval
```

Note: `jsr add` installs a **project dependency**, not a global binary. It does not put `doraval` on your PATH. Use the npm or Homebrew paths above for CLI use.
