...
Set up the journal (decision memory that remembers decisions and principles while scaling AI context) + agent integration with the recommended command:
...
`journal check` and Principle drift integration into `drift` / `validate` are the major remaining pieces (see the [implementation plan](https://github.com/saif-shines/doraval/blob/main/docs/specs/2026-05-25-journal-implementation-plan.md)).
...
- `dora drift` gains **Principle drift**: flags when a skill contradicts an active journal entry (planned).
