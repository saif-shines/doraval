...
1. **Minimal / magic (recommended for quick capture)**: just the title. If you ran `dora init`, the journal remembers decisions and principles while scaling AI context (skills, plugins, etc.); the configured agent can enrich entries with rich pushback/tags/rationale. Otherwise it uses the defaults above.
