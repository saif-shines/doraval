...
| `-r, --rationale` | If you want to supply a one-line rationale on the CLI. The old automatic editor for multi-line rationale is no longer triggered by default (edit the pending file before sync, or use --json for rich content). |
...
The entry is written as a single Markdown file with a `## Title` heading, a fenced YAML block, and the rationale body. The YAML always reflects the final values that were used (defaults, agent output, or explicit flags).
