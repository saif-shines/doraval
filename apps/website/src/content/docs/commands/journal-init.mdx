...
`journal init` is the prerequisite command. The full `journal` group also includes `list`, `update`, `add`, and `sync`. See the [design rationale](/concepts/journal-rationale) for why the architecture exists.
...
| `--refresh`       | Re-fetch files even if the project is already registered |
