...
A **routine** is a saved use case at `~/.dora/harness/<slug>/`. `dora harness` is the front desk. Hermes is the agent. Full flow: [dora harness](/commands/harness/). `dora harness new` starts `ask-dora` / `grilling-for-routine`. The family is `ask-dora`, `review-with-dora`, `grilling-for-routine`, and `writing-for-routine`.
