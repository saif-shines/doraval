...
Empty `dora` prints short `--help`. The first job is `dora review --quick`. The workspace map is `dora scan`. A pocket agent is a saved routine. Start it with `/ask-dora`. Model: [Pocket agent](/concepts/pocket-agent/). Walkthrough: [Run a pocket agent](/get-started/pocket-agents/). Apply: [Apply and watch](/get-started/apply-and-watch/).
...
dora review --run .          # no-skill then with-skill; exit 2 if no agent or Judge
