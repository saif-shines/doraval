...
A Recent install is younger than Install age (default 90 days). Unused names it. It is not a Remove candidate. Config: `review_window.last`, `review_window.max_age_days`, `install_age_days`.

A Plugin with some unused children lists those children. A Plugin with none invoked lists the Plugin only. Next on an Owned child is `dora skill remove <name> --dry-run`. Next on a Plugin row or an Imported child is `dora review --quick <plugin-root>`.
