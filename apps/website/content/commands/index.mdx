...
Sends hello to doraval.dev and waits for ack. Needs `identity.api_key`. A detected agent must pass `--yes` or `--dry-run`.
...
A detected agent must pass `--yes` or `--dry-run` to set a secret. `--deep` needs a Judge. Do not paste API keys into chat.
