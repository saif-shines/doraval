...
Empty `dora` prints short `--help`. The first job is `dora review --quick`. The workspace map is `dora scan`. A pocket agent is a saved routine. Start it with `/ask-dora`. Walkthrough: [Run a pocket agent](/get-started/pocket-agents/).
