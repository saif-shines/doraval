...
`--run-budget` is the one-pass time limit (default 10 minutes). Skills to run are passed as `--skills`. A pinned model adds `-m` and `--provider`. Dora refuses to overwrite an existing slug.
