...
`--run-budget` is the one-pass time limit (default 10 minutes). Local skills are copied into `.agents/skills` in a trusted workdir. The one-pass uses `--in` on that folder. Apply sets cron `--workdir` to the routine folder. Dora does not pass Hermes `--skills` names. A pinned model adds `-m` and `--provider`. Dora refuses to overwrite an existing slug.
