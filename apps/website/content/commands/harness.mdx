...
Apply also re-copies a skill when `routine.yml` records an upstream origin (skillkit, authstack, or a GitHub URL). A 15 minute tick does not do this. Pass `--keep-copies` to skip. A Fixed-step copy has no upstream origin, so apply leaves it. An older routine with no origin field is not refreshed unless you pass `--from <path|url>`.
