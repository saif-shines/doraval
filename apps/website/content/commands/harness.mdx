...
```bash
hermes chat --oneshot --run-budget 600 --reasoning xhigh -q "…"
```

`--run-budget` is the one-pass time limit (default 10 minutes). Local skills are copied into `.agents/skills` in a trusted workdir. The one-pass uses `--in` on that folder. Apply trusts the routine folder, inits a git root if missing, sets cron `--workdir`, and attaches each skills-run name with `--skill`. Hermes `skill_view` needs that git root. Dora does not pass Hermes `--skills` names. A pinned model adds `-m` and `--provider`. Dora refuses to overwrite an existing slug.

If the night job needs Hermes `delegate_task`, the grill checks `delegation.subagent_auto_approve`. When that key is not `true`, it prints `hermes config set delegation.subagent_auto_approve true` for you to run. The grill does not run that set. The key is machine-wide. It is not a routine field.
...
That command reads `~/.hermes`. It does not invent a catalog. The grill asks for model, provider, and reasoning from that list. Say **none** to follow the Hermes default. Pass `--model`, `--provider`, and `--reasoning-effort` on `--accept`.
...
Apply also installs a Hermes `pre_tool_call` hook. That hook stamps the same last line on the GitHub review and Slack post if the model skips it. A hook error does not block the post. A routine that has not been applied keeps the prompt line only. Restart a running Hermes gateway so the hook loads (`hermes gateway restart`).
...
A 15 minute tick does not refresh. After a skillkit or authstack merge, run apply again so the next tick reads `main`. Pass `--keep-copies` to skip. A Fixed-step copy has no upstream origin, so apply leaves it. An older routine with no origin field is not refreshed unless you pass `--from <path|url>`.
...
| Model | Hermes default (`model.default`) |
| Provider | Hermes default (`model.provider`) |
...
`apply` and `boot` pin `reasoning_effort` from `routine.yml` onto the Runtime job. Default is `xhigh`. Edit that field, then apply again, to change it. A missing field still pins `xhigh`, so an old folder does not fall back to medium.

`apply` and `boot` also pin `model` and `provider` when those fields are set. An empty `model` on apply clears the Hermes pin. A missing field leaves the Hermes job as it is. To change the pin later: run `dora harness models`, edit `routine.yml`, then apply again.
...
- Guessing a model id. Run `dora harness models` first.

## Related

- [Pocket agent](/concepts/pocket-agent/)
