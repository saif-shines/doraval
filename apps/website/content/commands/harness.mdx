...
A Tick has no last chat. The grill tells the night prompt to search the destination before a create, or to write `last_ts` in the routine folder. Dora does not write that file itself.
