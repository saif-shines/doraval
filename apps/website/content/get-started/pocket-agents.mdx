...
description: Grill a small loop, test one pass, then apply it. The folder is a Routine.
sidebar:
  label: Pocket agents
  order: 5
---
...
That is **loop-able**. Same prompt on a timer. No human on that run.
...
A Run has no last chat. Prefer a search before a create. GitHub approve is already safe. A calendar create is not. Search first, or use a **run handoff**: read `handoff.md` at the start, overwrite it at the end. Missing file means first Run. Dora does not write that file.
...
Do not put that line in `prompt.md`. Apply also installs a Hermes hook. That hook stamps `Sent by pocket agent slack-pr-approve` on the GitHub review and Slack post if the model skips it.
...
```bash
dora harness apply slack-pr-approve
hermes mcp login scalekit
```

`dora harness boot slack-pr-approve` is the same command. Apply fetches kit skills from GitHub `main`. Pass `--keep-copies` to skip. After a skillkit or authstack merge, run apply again. A tick does not refresh. Apply also installs the hook that stamps the last line on GitHub and Slack.
...
To avoid waiting an hour, edit `interval` in `~/.dora/harness/slack-pr-approve/routine.yml` to `1m` after a good run. Or say **run once** and fire one pass now.

Reasoning is `xhigh` in that same file (`reasoning_effort`). Apply pins it on the job. Change the field and apply again to keep a different level.

The same file can pin `model` and `provider`. Run `dora harness models` to see what this machine can use. Edit those fields, then apply again. You do not re-run the grill.
...
- Using `hermes logs` for one routine. Use `dora harness logs slack-pr-approve`.
