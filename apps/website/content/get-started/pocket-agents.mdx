...
`dora harness boot slack-pr-approve` is the same command. Apply fetches kit skills from GitHub `main`. Pass `--keep-copies` to skip. After a skillkit or authstack merge, run apply again. A tick does not refresh.
