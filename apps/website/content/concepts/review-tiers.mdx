...
A judge is either a **CLI you already have** (`claude` or `grok` on PATH — uses the subscription you already pay for) or an **API key** for an LLM provider (`dora config` / env). CLI is preferred; API is the fallback.
