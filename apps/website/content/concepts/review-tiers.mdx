...
`--run` feeds each `when` in [scenarios.yaml](/reference/scenarios/) to the configured coding agent. An LLM judge scores `expect` and `must_not`. FAIL is an error. Missing `scenarios.yaml` is fine.

`--run` needs an agent (`dora config set agent.command`) and an API judge. Exit `2` if either is missing. Do not mix `--quick` and `--run`.

The run uses an isolated HOME and cwd so host skills do not leak.
