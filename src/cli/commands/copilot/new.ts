/**
 * Thin wrapper — prefer `dora new --for copilot`.
 * Kept so existing `dora copilot new` invocations and tests keep working
...
export default defineCommand({
  meta: {
    name: "new",
    description: `Create a new skill or plugin for copilot (prefer: dora new --for copilot)`,
  },
  args: {
    name: {
      type: "positional",
  ...
  async run({ args }) {
    ui.heading(`doraval copilot new — use \`dora new --for copilot\` going forward`);
    const cwd = process.cwd();
    const ctx = detectScaffoldContext(cwd, "copilot");
