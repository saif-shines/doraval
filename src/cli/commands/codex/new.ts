 ...
 * Kept so existing `dora codex new` invocations and tests keep working
...
export default defineCommand({
  meta: {
    name: "new",
    description: `Create a new skill or plugin for codex (prefer: dora new --for codex)`,
  },
  args: {
    name: {
      type: "positional",
  ...
  async run({ args }) {
    ui.heading(`doraval codex new — use \`dora new --for codex\` going forward`);
    const cwd = process.cwd();
    const ctx = detectScaffoldContext(cwd, "codex");
    ...
    ui.info(`  Prefer: dora new skill --for codex --yes`);
    await exit(0);
  },
});
