...
export async function score(
  ...
  if (!existsSync(skillMd)) {
    console.error(
      `${chalk.red("✗")} No SKILL.md found at ${targetPath}`
    );
    process.exit(1);
  }
  ...
  // Check: uses imperative voice (heuristic: starts sections with verbs)
  const hasImperative = /\b(Create|Add|Run|Install|Configure|Set|Build|Use|Check|Verify|Ensure)\b/.test(
    body
  );
  checks.push({
    pass: hasImperative,
  ...
  if (options.format === "json") {
    ...
    console.error(
      `\n  ${chalk.bold("doraval")} v0.0.1 — Scoring skill\n`
    );

    for (const c of checks) {
      const icon = c.pass ? chalk.green("✓") : chalk.yellow("⚠");
