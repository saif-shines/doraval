...
async function reviewOne(path: string, opts: ReviewOptions = {}): Promise<ReviewResult> {
  const results = await review(path, opts);
...
const skipLlm = { judge: async () => ({ mode: "api" as const, ok: false as const, error: "skip" }) };

describe("review tier 4 wiring", () => {
  test("--quick omits the sessions tier entirely", async () => {
    const { root, dir } = skillFixture("t1");
    const r = await reviewOne(dir, { quick: true, cwd: root });
  ...
  test("no adapters → available:false (legacy shape)", async () => {
    ...
    const r = await reviewOne(dir, { quick: false, cwd: root, loadedSessions: NO_ADAPTERS, ...skipLlm });
  ...
  test("adapters but zero sessions → available:true, count 0, info finding", async () => {
    ...
    const r = await reviewOne(dir, { quick: false, cwd: root, loadedSessions: EMPTY_LOAD, ...skipLlm });
  ...
  test("--sessions with zero sessions throws E-PRE-003", async () => {
    ...
    await expect(
      reviewOne(dir, { quick: false, sessions: true, cwd: root, loadedSessions: EMPTY_LOAD, ...skipLlm })
  ...
  test("--sessions with zero adapters (no session-store agent installed) throws E-PRE-003", async () => {
    ...
    await expect(
      reviewOne(dir, { quick: false, sessions: true, cwd: root, loadedSessions: NO_ADAPTERS, ...skipLlm })
    ).rejects.toThrow(/No sessions found/);
    rmSync(root, { recursive: true, force: true });
  });
});
