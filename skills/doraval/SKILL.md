...
- Teammate wants a use case to keep running after chat → `dora harness new` (grill only). After the printed command: `dora harness new --accept --yes`. Then `dora harness boot <slug>`. Also `dora harness pause`, `dora harness resume`, `dora harness list`, `dora harness open`. A **routine** is `~/.dora/harness/<slug>/`.
