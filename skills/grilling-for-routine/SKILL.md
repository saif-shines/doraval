...
Hermes loads those copies only when the session cwd is that routine folder and the folder is a git root. The one-pass runs `hermes skills trust` and `hermes chat --in` on a staging folder. Apply trusts the routine folder, inits a git root if missing, sets cron `--workdir`, and attaches each skills-run name with `--skill`. Without a git root, `skill_view` cannot load a skills-run copy.
