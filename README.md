...
dora memory list
dora memory promote            # write weight ≥ 7 principles into AGENTS.md (diff + confirm)
dora memory promote --dry-run  # show the planned AGENTS.md section only
...
High-weight principles show up as errors in every future `dora review` on this project. `memory promote` compiles them into a managed AGENTS.md section so every agent shares the same hard rules. Stashed artifacts survive a clean clone — handy for local notes, scratch configs, or anything git deliberately ignores but you don't want to lose. Warns above 5MB per file, refuses above 50MB (use git-lfs instead).
